module chat_room::chat_room {
    ...
    public entry fun join_chat_room(
        sender: &signer, room_address: address, username: String
    ) acquires ChatRoom {
        ...
        assert!(
            user_index == smart_vector::length(&chat_room.users), EUSER_ALREADY_EXISTS
        );

        smart_vector::push_back(
            &mut chat_room.users,
            User { user_address: sender_address, username }
        );

        event::emit(
            JoinedChatRoom { sender: sender_address, room_address, username }
        );
    }

    public entry fun send_chat(
        sender: &signer, room_address: address, message: String
    ) acquires ChatRoom {
        assert!(exists<ChatRoom>(room_address), ECHAT_ROOM_DOES_NOT_EXIST);
        ...
        let user_index = get_user_index(chat_room, sender_address);
        assert!(
            user_index != smart_vector::length(&chat_room.users), EUSER_DOES_NOT_EXIST
        );

        smart_vector::push_back(
            &mut chat_room.messages,
            Message { message, reacts: 0 }
        );
    ...
    public entry fun react_to_message(
        sender: &signer, room_address: address, message_index: u64
    ) acquires ChatRoom {
        assert!(exists<ChatRoom>(room_address), ECHAT_ROOM_DOES_NOT_EXIST);
        ...
        event::emit(
            SentReaction { sender: sender_address, username, message: message.message }
        );
    }

    public entry fun leave_chat_room(
        sender: &signer, room_address: address
    ) acquires ChatRoom {
        let sender_address = signer::address_of(sender);
        assert!(exists<ChatRoom>(room_address), ECHAT_ROOM_DOES_NOT_EXIST);
        ...
        event::emit(
            LeftChatRoom { sender: sender_address, room_address, username: user.username }
        );
    }
